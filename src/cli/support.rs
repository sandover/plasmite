...
pub(crate) fn validate_secret_file(path: &Path, kind: &str) -> Result<(), Error> {
    ...
    #[cfg(windows)]
    {
        let script = r#"$ErrorActionPreference='Stop'; Import-Module Microsoft.PowerShell.Security; $acl=Get-Acl -LiteralPath $env:PLASMITE_ACL_PATH; $current=[Security.Principal.WindowsIdentity]::GetCurrent().User.Value; $groups=& "$env:SystemRoot\System32\whoami.exe" /groups; $logons=@([regex]::Matches(($groups -join "`n"),'S-1-5-5-\d+-\d+') | ForEach-Object { $_.Value }); $allowed=@($current,'S-1-5-18','S-1-5-32-544')+$logons; $allows=@($acl.Access | Where-Object { $_.AccessControlType -eq 'Allow' } | ForEach-Object { try { $_.IdentityReference.Translate([Security.Principal.SecurityIdentifier]).Value } catch { 'unresolved' } }); $bad=@($allows | Where-Object { $_ -notin $allowed }); if ($acl.AreAccessRulesProtected -and $bad.Count -eq 0 -and $current -in $allows) { exit 0 } else { exit 3 }"#;
