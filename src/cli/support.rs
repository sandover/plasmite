...
pub(crate) fn validate_secret_file(path: &Path, kind: &str) -> Result<(), Error> {
    ...
    #[cfg(windows)]
    {
        let script = r#"$ErrorActionPreference='Stop'; Import-Module Microsoft.PowerShell.Security; $acl=Get-Acl -LiteralPath $env:PLASMITE_ACL_PATH; $identity=[Security.Principal.WindowsIdentity]::GetCurrent(); $principal=[Security.Principal.WindowsPrincipal]::new($identity); $current=$identity.User.Value; $allowed=@($current,'S-1-5-18','S-1-5-32-544'); $allows=@($acl.Access | Where-Object { $_.AccessControlType -eq 'Allow' } | ForEach-Object { try { $_.IdentityReference.Translate([Security.Principal.SecurityIdentifier]).Value } catch { 'unresolved' } }); $bad=@($allows | Where-Object { if ($_ -in $allowed) { $false } elseif ($_ -match '^S-1-5-5-\d+-\d+$') { -not $principal.IsInRole([Security.Principal.SecurityIdentifier]::new($_)) } else { $true } }); if ($acl.AreAccessRulesProtected -and $bad.Count -eq 0 -and $current -in $allows) { exit 0 } else { exit 3 }"#;
