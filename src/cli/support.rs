...
pub(crate) fn validate_secret_file(path: &Path, kind: &str) -> Result<(), Error> {
    ...
    #[cfg(windows)]
    {
        let script = r#"$ErrorActionPreference='Stop'; Import-Module Microsoft.PowerShell.Security; $acl=Get-Acl -LiteralPath $env:PLASMITE_ACL_PATH; $identity=[Security.Principal.WindowsIdentity]::GetCurrent(); $current=$identity.User.Value; $logons=@($identity.Groups | ForEach-Object { $_.Value } | Where-Object { $_ -match '^S-1-5-5-\d+-\d+$' }); $allowed=@($current,'S-1-5-18','S-1-5-32-544')+$logons; $allows=@($acl.Access | Where-Object { $_.AccessControlType -eq 'Allow' } | ForEach-Object { try { $_.IdentityReference.Translate([Security.Principal.SecurityIdentifier]).Value } catch { 'unresolved' } }); $bad=@($allows | Where-Object { $_ -notin $allowed }); if ($acl.AreAccessRulesProtected -and $bad.Count -eq 0 -and $current -in $allows) { exit 0 } else { exit 3 }"#;
