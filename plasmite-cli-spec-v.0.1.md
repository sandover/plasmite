...
* `--pretty` : pretty-printed JSON (multi-line)
...
* `--since 10s|5m|2h` (best-effort; depends on timestamps)
* `--follow` (block and continue)
* `--idle-timeout 30s` (exit if nothing arrives in that time)
...
* **ts**: timestamp (stored as ns; rendered as RFC3339 in CLI JSON)
...
Get oldest/newest seq (and optionally count).

**Synopsis**

```bash
plasmite pool bounds NAME [--oldest] [--newest] [--count]
```

Defaults:

* without flags prints a JSON object to stdout: `{oldest,newest,count}` (fields may be omitted if unavailable/expensive)
* with `--oldest`/`--newest`/`--count`, emit only those keys in the JSON object

This replaces `p-oldest-idx` / `p-newest-idx` and makes scripting nicer.
...
* Filtering (lightweight, not a query language):
  ...
  * `--descrip HAS` (repeatable; match any `meta.descrips[]` by default)
  * `--match JSONPOINTER=VALUE` (optional; minimal; e.g. `/data/type=foo`)
...
### 1) Tail a pool like `peek`

```bash
plasmite peek mypool --follow
```

### 2) Pipe into `jq`

```bash
plasmite peek mypool --follow --jsonl | jq -r '.meta.descrips[]?'
```
...
* `p-oldest-idx` / `p-newest-idx` collapse into `pool bounds` (better scripting).
* `p-await` becomes `peek --follow` (no separate concept).
