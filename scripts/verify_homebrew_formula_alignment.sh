...
# Invariants: Formula URLs and sha256 entries for every manifest Homebrew target must match.
...
done < <("$(dirname "${BASH_SOURCE[0]}")/release_channel_targets.sh" homebrew official sdk_platform)
